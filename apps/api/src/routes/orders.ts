...
import { GSTCalculator } from '@eazyque/shared';
...
const createOrderSchema = z.object({
  ...
  paymentMethod: z.enum(['CASH', 'UPI', 'CARD', 'WALLET', 'SPLIT']),
  discountAmount: z.number().min(0).optional().default(0),
...
// Update order status
router.patch('/:id/status', authMiddleware, requireStaff, async (req, res) => {
  try {
    const { shopId } = req.user!;
    const { id } = req.params;
...
// Cancel order
router.patch('/:id/cancel', authMiddleware, requireStaff, async (req, res) => {
  try {
    const { shopId } = req.user!;
    const { id } = req.params;
